...
function createGrid(gridSize) {
  ...
  for (let i = 0; i < gridSize; i++) {
    ...
    for (let j = 0; j < gridSize; j++) {
      ...
      div.addEventListener("click", changeCellColor);
...
canvas.addEventListener("touchmove", clickCellAtPoint);

// for normal mouse interactive devices
canvas.addEventListener("mousedown", () => {
  canvas.addEventListener("mousemove", clickCellAtPoint);
});
canvas.addEventListener("mouseup", () => {
  canvas.removeEventListener("mousemove", clickCellAtPoint);
});
